...
(slide,從)——簡報由確定性**摘要投影**自動降密度而來(每段取第一句、
連續段落收斂為要點、形狀區塊原樣),零 LLM。`<!-- slide: skip/keep/自訂
要點 -->` 可逐段控制兩態呈現。「✦ AI 產生 → 引導我寫 blog」會給你一份
對話式意圖澄清 prompt,帶去你自己的 LLM 產出母體。
...
- **v2 雙態控制**:`<!-- slide: skip -->`(只在文件態)、`<!-- slide: keep -->`(整段上台)、`<!-- slide: 自訂要點 -->`(簡報顯示要點、文件顯示全文);寫作訣竅——**每段第一句就是要點**(≤30 字),簡報會自動抽它。
